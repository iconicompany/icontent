...
- **Дообучение CrossEncoder не сошлось.** Loss встал на месте, а качество на held-out даже деградировало. Причина оказалась не в learning rate, а в данных: у нас есть позитивы (сделанные офферы), но нет *размеченных негативов* — «этот кандидат вакансии не подходит». Намайненные негативы — топически близкие кандидаты, часть из которых на самом деле нормальные матчи, просто не обработанные. Модель получала противоречивый сигнал. Важный урок: **сломанный прогон обучения — это не валидный отрицательный результат**, его нельзя выдавать за «CrossEncoder проигрывает».
