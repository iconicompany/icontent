---
title: 'Методы извлечения навыков из резюме и вакансий'
date: '2026-03-06'
description: 'Разбираем конвейер автоматического извлечения навыков: NER на RuBERT, сопоставление шаблонов через spaCy PhraseMatcher и нормализация через векторные представления SentenceTransformers.'
...
В этой статье рассматривается конвейер, реализованный в <a href="https://github.com/iconicompany/iskillmatching">`iskillmatching`</a>, который объединяет три взаимодополняющих подхода:
...
В `ner_utils.py` применяется пайплайн HuggingFace Transformers:
...
Оба метода дают пересекающиеся, но дополняющие друг друга результаты. В `clean_utils.py` они объединяются и очищаются:
...
В `normalize_utils.py` используется библиотека **SentenceTransformers**:
...
Если порог не достигнут, навык отбрасывается - это защита от ложных обобщений (например, чтобы слово "работа" не нормализовалось к "Java"). Класс `SkillNormalizer` принимает порог как параметр конструктора (по умолчанию `0.6`), а в точке вызова в `extract.py` он переопределяется до `0.8` для получения более строгих совпадений в продакшн-сценарии.
...
```bash
# Установка зависимостей (spaCy модель для токенизации)
...
Можно попробовать <a href="https://github.com/iconicompany/NER-Web-App-TensorFlowJS/pull/1">обучение собственной модели NER на основе NER-Web-App-TensorFlowJS</a>
или настройку <a href="https://github.com/MarcusElwin/ner-dspy">prompt к LLM через DSPy</a>.

1. Сравнение различных вариантов NER: <a href="https://arxiv.org/pdf/2407.19816">arxiv.org/pdf/2407.19816</a>, модель из статьи: <a href="https://huggingface.co/dondosss/rubert-finetuned-ner">dondosss/rubert-finetuned-ner</a>
2. Модель NER на tensorflow: <a href="https://github.com/mrstelmach/NER-Web-App-TensorFlowJS">NER-Web-App-TensorFlowJS</a>
3. SentenceTransformer based on all-mpnet-base-v2: <a href="https://huggingface.co/TechWolf/JobBERT-v2">TechWolf/JobBERT-v2</a>
4. SentenceTransformer based on all-MiniLM-L6-v2: <a href="https://huggingface.co/anass1209/resume-job-matcher-all-MiniLM-L6-v2">anass1209/resume-job-matcher-all-MiniLM-L6-v2</a>
5. Ещё один SentenceTransformer based on all-MiniLM-L6-v2: <a href="https://huggingface.co/hetbhagatji09/Job-resume-ner-match-model">hetbhagatji09/Job-resume-ner-match-model</a>
6. Сервис для запуска моделей: <a href="https://github.com/michaelfeil/infinity">michaelfeil/infinity</a>
7. NER на LLM с DSPy: <a href="https://github.com/MarcusElwin/ner-dspy">MarcusElwin/ner-dspy</a>

Исходный код конвейера: <a href="https://github.com/iconicompany/iskillmatching">iconicompany/iskillmatching</a>
