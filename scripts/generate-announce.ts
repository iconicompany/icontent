...
export const PROMPTS = {
  ...
  regular: {
    ru: "Напиши развернутый пост по мотивам этой статьи. Весь текст должен быть строго до 3000 символов (это жесткое ограничение). Пиши живым, но профессиональным языком, избегай корпоративных клише. Структурируй текст с помощью абзацев, списков и обязательно добавь подходящие эмодзи по смыслу для визуальных акцентов, чтобы его было удобно читать. Не используй markdown-разметку (никаких **, *, #). Не используй хэштеги. Не добавляй ссылку — она будет добавлена автоматически. Отвечай только текстом поста, без пояснений и кавычек.",
